...
namespace Utils
{
    inline Ray ScreenPointToRay(const glm::vec2& uv, const glm::mat4& cam_matrix)
    {
        ...
        const glm::mat4 inverseViewProjection = glm::inverse(cam_matrix);
        const glm::vec4 nearRay = inverseViewProjection * screenSpace;
        ...
        const glm::vec4 farRay = inverseViewProjection * screenSpace;
        const glm::vec3 rayDirection = glm::normalize(glm::vec3(farRay) / farRay.w - rayOrigin);

        return {glm::vec3(rayOrigin), rayDirection};
    }
}
